...
class StaticAdminUiTests(unittest.TestCase):
    ...
    def test_admin_ui_exposes_invoice_preview_save_and_export_controls(self):
        ...
        self.assertIn("format=${format}&view=${view}", admin_html)
        self.assertIn("exportInvoice('customer', 'csv')", admin_html)
        self.assertIn("exportInvoice('internal', 'csv')", admin_html)
        self.assertIn("exportInvoice('customer', 'xlsx')", admin_html)
        self.assertIn("exportInvoice('customer', 'pdf')", admin_html)
        self.assertIn("exportInvoice('internal', 'xlsx')", admin_html)
    ...
    def test_admin_ui_exposes_customer_project_endpoint_map_controls(self):
        ...
        self.assertIn("预检（不创建）", admin_html)
        self.assertIn("自动补齐缺失 Endpoints", admin_html)
        self.assertIn("endpointProvisionActionLabel", admin_html)
        self.assertIn("预检已完成（未创建资源）", admin_html)
        self.assertIn("已有映射会复用，不会重建", admin_html)
        self.assertIn("Endpoint Map Health", admin_html)
        self.assertIn("/admin/upstream/endpoint-map-health", admin_html)
        self.assertIn("Quota Center Reminders", admin_html)
        self.assertIn("/admin/upstream/quota-reminders", admin_html)
        self.assertIn("endpoint_key_map_configured", admin_html)
